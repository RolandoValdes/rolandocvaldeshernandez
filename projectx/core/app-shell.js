class ProjectXApp {
  ...
  renderNavigation() {
    ...
    if (!this.navigationItems.length) {
      this.navTarget.innerHTML = `<div class="module-error">No navigation configured for this module.</div>`;
      return;
    }
    this.navTarget.innerHTML = this.navigationItems.map(item => {
      const isActive = item.view === this.currentView ? "active" : "";
      return `<button class="module-nav-item ${isActive}" data-view="${this.escape(item.view)}">${this.escape(item.label || item.title || item.view)}</button>`;
    }).join("");
  }
  registerEvents() {
    if (!this.navTarget) return;
    this.navTarget.addEventListener("click", event => {
      ...
      this.navTarget.querySelectorAll(".module-nav-item").forEach(item => item.classList.remove("active"));
  ...
  renderView(view) {
    ...
    if (this.moduleTitleTarget) this.moduleTitleTarget.textContent = title;
    const customRendererName = `${this.moduleKey}_${view}`.replace(/-/g, "_");
    if (window.ProjectXViews && typeof window.ProjectXViews[customRendererName] === "function") {
      this.workspaceTarget.innerHTML = window.ProjectXViews[customRendererName](this);
      return;
    }
    this.workspaceTarget.innerHTML = this.defaultViewTemplate(title, view);
  }
  defaultViewTemplate(title, view) {
    return `<div class="projectx-card"><span class="projectx-status">${this.escape(this.moduleTitle)}</span><h2>${this.escape(title)}</h2><p>This workspace view is ready for development inside Project X.</p><div class="projectx-grid" style="margin-top:20px;"><div class="projectx-card"><h3>Workspace</h3><p>Design the main tools and content for the ${this.escape(title)} view.</p></div><div class="projectx-card"><h3>Data</h3><p>Connect this view to future JSON, forms, tables, dashboards or AI tools.</p></div><div class="projectx-card"><h3>Status</h3><p>Module key: <strong>${this.escape(this.moduleKey)}</strong><br>View key: <strong>${this.escape(view)}</strong></p></div></div></div>`;
  }
  toTitle(value) { return String(value || "").replace(/-/g, " ").replace(/\b\w/g, char => char.toUpperCase()); }
